...
- `--agents` / `--agentsN`：列出指定主会话下的子代理 rollout，并展示任务路径、首条和最后一条子代理回复摘要
- `--agent <n|agentPath>` / `--agentN`：按子代理编号或 `/root/...` 路径读取子代理对话；可与 `--watch` 组合跟随输出

### Changed

- 子代理列表按自身首个 `turn_context` 之后的消息生成摘要，避免把继承的父会话历史误展示为子代理任务
- 子代理发现只读取无关 rollout 的会话元数据，再完整解析匹配项，缩短大型会话目录中的列表耗时

### Fixed

- `--watch` 现在会补发监听器建立前追加的事件，避免初始输出与监听启动之间丢失消息
